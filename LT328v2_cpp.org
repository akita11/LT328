...
void LT::com_process(void)
{
  if(LT::com_operation == 1){
	...
	else if (com_tst[dir] == 2){
	  if (com_txd[dir] == 1) com_set_pin(dir, 1);
	  else com_set_pin(dir, 0);
	  com_tst[dir] = 3;
	}
	else if (com_tst[dir] == 3){
	  com_tst[dir] = 4;
	}
	else if (com_tst[dir] == 4){
	  com_set_pin(dir, 1);
	  com_tst[dir] = 0;
	}
      }
	...
	else if (com_rst[dir] == 1){
	  com_rst[dir] = 2;
	}
	else if (com_rst[dir] == 2){
	  com_rbbuf[dir][com_pbb[dir]] = com_read_pin(dir);
	  com_rst[dir] = 3;
	}
	else if (com_rst[dir] == 3){
	  //	  if (com_read_pin(dir) == 1){ // check last '1' in each bit
	  com_rbuf[dir][com_pb[dir]] = com_rbuf[dir][com_pb[dir]] >> 1;
	  if (com_rbbuf[dir][com_pbb[dir]] == 1)
	    com_rbuf[dir][com_pb[dir]] |= 0x80;
	  else
	    com_rbuf[dir][com_pb[dir]] &= ~0x80;

	  com_rst[dir] = 0;
	  com_pbb[dir]++;
	  if (com_pbb[dir] == COM_RXBUF) com_pbb[dir] = 0;
...
void LT::com_write_bit(uint8_t dir, uint8_t d)
{
  while(com_tst[dir] != 0);
  while(com_rst[dir] != 0);
  com_txd[dir] = d;
...
int LT::com_available_bit(uint8_t dir)
{
  if (com_prb[dir] == com_pbb[dir]) return(0);
  else if (com_prb[dir] < com_pbb[dir]) return(com_pbb[dir] - com_prb[dir]);
  else return(com_pbb[dir] + COM_RXBUF - com_prb[dir]);
}

uint8_t LT::com_read_bit(uint8_t dir)
{
  uint8_t d;
  d = com_rbbuf[dir][com_prb[dir]];
  com_prb[dir]++;
  if (com_prb[dir] == COM_RXBUF) com_prb[dir] = 0;
  return(d);
}
...
void LT::com_write(uint8_t dir, uint8_t d)
{
  for (uint8_t b = 0; b < 8; b++){
    if (d & (1 << b)) com_write_bit(dir, 1); else com_write_bit(dir, 0);
  }
}
